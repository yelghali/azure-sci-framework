...
def get_grid_carbon_intensity_forecast_i():
    # Get the 24-hour forecast for I from the carbon intensity configmap
    configmap_name = 'carbonintensity'
    configmap_namespace = 'kube-system'
    configmap = api_client.read_namespaced_config_map(configmap_name, configmap_namespace)
    configmap_data = configmap.data
    forecast_data = json.loads(configmap_data['data'])
    forecast_i = forecast_data['forecast'][0]['intensity']

    return forecast_i

def get_all_nodes_usage_telemetry():
    ...
    for item in node_metrics['items']:
        ...
        cpu_energy = calculate_node_cpu_energy(cpu_usage_percentage, float(tdp))
...
def calculate_node_cpu_energy(cpu_usage_percentage, tdp):
...
def calculate_node_memory_energy(memory_usage_percentage):
...
def calculate_node_embodied_emissions_m(node_sku):
...
def calculate_node_total_energy():
    # Get the usage telemetry for all nodes
    node_usage_telemetry = get_all_nodes_usage_telemetry()
    ...
    for node_name, node_info in node_usage_telemetry.items():
        ...
        cpu_energy = calculate_node_cpu_energy(node_info['cpu_percentage'], float(tdp))
        node_info['cpu_energy'] = cpu_energy

    # Calculate the memory energy consumption for each node
        memory_energy = calculate_node_memory_energy(node_info['memory_percentage'])
...
def calculate_node_sci_metrics():
    sci_metrics = {}
    return sci_metrics

def export_node_sci_metrics():
    ...
    node_energy = calculate_node_total_energy()
    print(node_energy)
    print("######\n")

    sci_metrics = calculate_node_sci_metrics()
    print(sci_metrics)
